...
public partial class IndexEnrolling
{
    ...
    private async Task Cargar(int page = 1)
    {
        ...
        var responseHttp = await _repository.GetAsync<List<Enrolling>>(url);
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        if (isEdit)
        {
            ...
            {
                { "Id", id },
                { "Title", "Edit Enrolling"  }
            };
            await _modalService.ShowAsync<EditEnrolling>(parameters);
        }
        else
        {
            var parameters = new Dictionary<string, object>
            {
                { "Title", "Create Enrolling"  }
            };
            await _modalService.ShowAsync<CreateEnrolling>(parameters);
        }
    }
