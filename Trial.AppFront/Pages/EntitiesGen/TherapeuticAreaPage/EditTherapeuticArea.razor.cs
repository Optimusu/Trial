...
public partial class EditTherapeuticArea
{
    //Services
    ...
    private TherapeuticArea? _therapeuticArea;
    private const string BaseUrl = "/api/v1/therapeutics";
    private const string BaseView = "/therapeutics";

    protected override async Task OnInitializedAsync()
    {
        var responseHttp = await _repository.GetAsync<TherapeuticArea>($"{BaseUrl}/{Id}");
        if (await _responseHandler.HandleErrorAsync(responseHttp)) return;

        _therapeuticArea = responseHttp.Response;
    }

    private async Task Edit()
    {
        var responseHttp = await _repository.PutAsync($"{BaseUrl}", _therapeuticArea);
