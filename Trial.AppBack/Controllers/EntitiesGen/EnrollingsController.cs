...
{
    [ApiVersion("1.0")]
    [Route("api/v{version:apiVersion}/enrollings")]
    [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "Admin, Administrator")]
    [ApiController]
    public class EnrollingsController : ControllerBase
    {
        private readonly IEnrollingUnitOfWork _unitOfWork;
        private readonly IStringLocalizer _localizer;

        public EnrollingsController(IEnrollingUnitOfWork unitOfWork, IStringLocalizer localizer)
        {
        ...
        [HttpPut]
        public async Task<IActionResult> PutAsync(Enrolling modelo)
        {
            try
            {
        ...
        [HttpPost]
        public async Task<IActionResult> PostAsync(Enrolling modelo)
        {
            try
            {
